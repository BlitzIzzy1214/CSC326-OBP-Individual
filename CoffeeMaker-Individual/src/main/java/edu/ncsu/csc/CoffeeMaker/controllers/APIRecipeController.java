...
public class APIRecipeController extends APIController {
    ...
    public ResponseEntity createRecipe ( @RequestBody final Recipe recipe ) {
        ...
        if ( service.findAll().size() < 3 ) {
            if ( recipe.getAmounts().size() == 0 || recipe.getIngredients().size() == 0 ) {
                return new ResponseEntity( errorResponse( "Recipe must contain ingredients" ), HttpStatus.BAD_REQUEST );
            }
            service.save( recipe );
     ...
     *
     * @param recipe
     *            The valid Recipe to be updated.
     * @return ResponseEntity indicating success if the Recipe could be saved to
     *         the inventory, or an error if it could not be
     */
    ...
    public ResponseEntity updateRecipe ( @RequestBody final Recipe recipe ) {
        ...
        service.delete( service.findByName( recipe.getName() ) );
        service.save( recipe );
